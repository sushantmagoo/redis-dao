...
const findAll = async () => {
  // START CHALLENGE #1
  const client = redis.getClient();
  let all_sites = await client.smembersAsync(keyGenerator.getSiteIDsKey());
  let result = [];

  for (let site of all_sites) {
    let data = await client.hgetallAsync(site);
    result.push(remap(data));
  }
